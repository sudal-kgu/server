...
public class DeadLetterConsumer {
    ...
    @RabbitListener(queues = "${rabbitmq.dead-letter-queue-name}")
    public void consume(AnalysisResponseDto analysisResponseDto) {
        log.error("[MQ] Failed to process result: requestId={}", analysisResponseDto.getRequestId());
        analysisResultNotifier.notifyError(analysisResponseDto.getRequestId(), ErrorStatus.INTERNAL_SERVER_ERROR);
    }
}